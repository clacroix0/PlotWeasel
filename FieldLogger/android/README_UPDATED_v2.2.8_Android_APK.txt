...
Purpose:
This is an offline field data-entry app for Samsung tablets, Android field tablets, Microsoft Surface devices, and secured Windows computers. It collects plot, tree, and regeneration records, then exports files for the Master Workbook and Plot Weasel Desktop.
...
Recommended Android launch path:
1. Copy "PlotWeaselFieldLogger_Ver2.2.8.apk" to the Android tablet.
2. On the tablet, open Files or My Files and tap the APK.
3. If Android asks, allow installation from that source, such as Files, My Files, Chrome, Drive, or the approved file manager.
4. Install the app and open Plot Weasel Field Logger from the app icon.
5. Allow Location permission while using the app. Use Precise location if Android offers that option.
6. Confirm the app header shows "Local only" before field use.
7. Create a test project, close and reopen the app, and confirm the test project is still there.
8. Test Backup JSON and Plot Weasel Desktop CSV export before collecting real field data.

Supported non-APK launch path:
1. Unzip the Field Logger folder.
2. On Windows or Microsoft Surface, open "PlotWeasel_Field_Logger_v2.2.8.html" in Microsoft Edge.
...
Storage mode:
- Local only: expected mode for field use. Data is stored locally on the device until exported or cleared.
- Memory only: not safe for field use. Data may disappear when the app closes, reloads, crashes, or Android clears it.
- Do not use a build that shows "Memory only" for real field collection unless persistent storage has been separately fixed and tested.
- The official Android Studio-built APK is the recommended Android route because it was tested to show "Local only".

Browser/device notes:
- Microsoft Edge is the supported browser for the offline HTML version on Windows and Surface devices.
- Chrome may work on some devices, but Edge is the tested path for loose local HTML files.
- A loose .html file is treated as a document on Android. For Android field tablets, use the official APK when possible.
- Safari on iPhone/iPad may treat local HTML as a preview instead of a full app, so Species, Trees, Regen, Add Plot, Aspect, or Slope Position may not work there. Test hard before relying on iPhone/iPad.
- The official Android APK does not require Google Play Store publishing or a Google Play Developer account for direct/internal installation.
...
3. Copy the Project Setup JSON to each receiving tablet by approved local transfer. Also install the APK or copy the Field Logger folder if the tablet does not already have Field Logger.
...
7. Each crew should also export a Backup JSON at the end of each day.
8. Back at the office, open Field Logger on a secured computer.
9. Import all crew packages in Export / Merge.
10. Export Site / Tree / Regen CSVs for reference/master records and Plot Weasel Desktop CSV for the desktop upload.
11. Load or paste Site.csv, Tree.csv, and Regen.csv into the Master Workbook as needed. Upload only PlotWeasel_upload.csv into Plot Weasel Desktop.

Security notes:
- The app does not require internet, cloud storage, Python, R, or admin rights for normal use.
- GPS capture uses the device location service if the app or browser allows it. It does not upload coordinates or require cloud sync.
- Data is stored in local device/browser/app storage until exported or cleared.
- Local storage is not encrypted by this app. Keep the tablet secured and clear device data after confirmed transfer if policy requires it.
- Do not use cloud folders for crew packages if the data is sensitive unless policy allows it.

Updating or uninstalling:
- Before installing an updated APK, export Backup JSON and confirm the file was saved.
- Do not uninstall the app unless data has been backed up. Uninstalling can remove local project data.
- APK installer files in Downloads can be deleted after installation. Deleting the APK installer file does not delete the installed app or its saved data.
- Future APK updates must be signed with the same Android signing key to install cleanly over this release.
- Keep the .jks signing key file, keystore password, key alias, and key password private and backed up. Do not commit them to GitHub.

Projects on one device:
- Field Logger supports multiple saved projects in the same browser or app storage on the same tablet.
...
- Clear Device Data removes all saved projects from that app/browser on that device.
...
- Backup JSON: full backup of current device data. Export this before updating, uninstalling, clearing device data, or moving work to another tablet.
...
- The Site tab can capture plot-center GPS for 30 seconds, average the fixes, convert the average to whole-meter UTM Easting/Northing/Zone, and write those values to Site.csv. If the device returns altitude, the same capture also fills Elevation as whole feet. Location services and app/browser permission must be allowed. UTM and Elevation can also be typed manually. GPS Fix Count is the number of readings collected during the 30-second capture.
...
The MIT App Inventor WebViewer wrapper can open the Field Logger, but the simple WebViewer build may show "Memory only" and should not be used for real field collection unless persistent storage has been separately fixed and tested. The Android Studio-built APK is the recommended Android APK route for v2.2.8 field testing.
